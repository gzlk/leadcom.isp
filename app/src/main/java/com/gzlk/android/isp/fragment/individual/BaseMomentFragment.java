...
public abstract class BaseMomentFragment extends BaseChatInputSupportFragment {
     ...
     * 查看我是否已赞过
     */
    protected void checkIsMyPraised() {
        LikeRequest.request().setOnSingleRequestListener(new OnSingleRequestListener<ArchiveLike>() {
            ...
            public void onResponse(ArchiveLike archiveLike, boolean success, String message) {
                ...
                onCheckIsMyPraisedComplete(success);
            }
     ...
     * 赞某个说说
     */
    protected void praiseMoment() {
        LikeRequest.request().setOnSingleRequestListener(new OnSingleRequestListener<ArchiveLike>() {
            @Override
            public void onResponse(ArchiveLike archiveLike, boolean success, String message) {
                super.onResponse(archiveLike, success, message);
                onPraiseMomentComplete(archiveLike, success);
            }
     ...
     * 取消赞
     */
    protected void deletePraiseMoment() {
        LikeRequest.request().setOnSingleRequestListener(new OnSingleRequestListener<ArchiveLike>() {
            @Override
            public void onResponse(ArchiveLike archiveLike, boolean success, String message) {
                super.onResponse(archiveLike, success, message);
                onDeletePraiseMomentComplete(success);
            }
